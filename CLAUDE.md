...
Pushing a tag matching `v[0-9]+.[0-9]+.[0-9]+` (e.g. `v0.1.1`) triggers
`.github/workflows/release.yml`, which runs `go test ./...`, then
`go run ./tools/build --release`, packages each platform's binaries
(`.zip` for `windows_*`, `.tar.gz` otherwise) as
`mdutils_<tag>_<goos>_<goarch>.{zip,tar.gz}`, and publishes a GitHub Release
with those archives attached — using the matching `## [x.y.z]` section of
`CHANGELOG.md` as the release notes when one exists, falling back to
autogenerated notes otherwise. Update `CHANGELOG.md` with the new version's
section *before* tagging, so the extracted section is meaningful. No manual
build/package/upload step is needed after tagging.
